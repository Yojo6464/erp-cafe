...
def consultar_balance():
    try:
        ...
        for fila in registros:
            saldo_neto = fila["debitos"] - fila["creditos"]

            if fila["naturaleza"] == "DEBITO":
                saldo_anormal = saldo_neto < -0.005
            else:
                saldo_anormal = saldo_neto > 0.005
            ...
            fila["estado_saldo"] = estado_saldo
        ...
        lbl_anormales.config(
            text=f"Saldos anormales: {cuentas_anormales}",
            fg=COLOR_ROJO if cuentas_anormales else COLOR_VERDE
        )

        if abs(diferencia) <= 0.01:
            if cuentas_anormales:
                lbl_estado.config(
                    text="ECUACIÓN CUADRADA · REVISAR SALDOS ANORMALES",
                    fg=COLOR_ROJO
                )
            else:
                lbl_estado.config(
        ...
        resumen_actual.update({
            ...
            "cuentas_anormales": cuentas_anormales
        })
...
lbl_anormales = tk.Label(
    estado,
    text="Saldos anormales: 0",
    bg=COLOR_TARJETA,
    fg=COLOR_SUAVE,
    font=("Segoe UI", 9, "bold")
)
lbl_anormales.pack(side="right", padx=(6, 22))
...
tk.Label(
    panel_tabla,
    text=(
        "NORMAL: el saldo coincide con la naturaleza de la cuenta. "
        "ANORMAL: débitos y créditos dejaron la cuenta con naturaleza invertida."
    ),
    bg=COLOR_TARJETA,
    fg=COLOR_SUAVE,
    font=("Segoe UI", 8)
