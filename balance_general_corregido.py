...
def consultar_balance():
    try:
        ...
        lbl_total_activo.config(text=moneda(total_activo))
        lbl_total_pasivo.config(text=moneda(total_pasivo))
        lbl_total_patrimonio.config(text=moneda(total_patrimonio))
        lbl_pasivo_patrimonio.config(
            text=moneda(total_pasivo_patrimonio)
        )
        lbl_capital_trabajo.config(text=moneda(capital_trabajo))
        lbl_razon_corriente.config(text=f"{razon_corriente:,.2f}")

        lbl_activo_corriente.config(text=moneda(activo_corriente))
        lbl_activo_no_corriente.config(
            text=moneda(activo_no_corriente)
        )
        lbl_pasivo_corriente.config(text=moneda(pasivo_corriente))
        lbl_pasivo_no_corriente.config(
            text=moneda(pasivo_no_corriente)
        )
