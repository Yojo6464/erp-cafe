...
class MotorOrdenesProduccion:
    ...
    def _validar_estructura_minima(self) -> None:
        requeridas = {"productos_produccion", "formulas_produccion", "formulas_componentes", "ordenes_produccion_v2", "ordenes_historial_v2", "ordenes_requerimientos_v2", "inventario"}
    ...
    def crear_orden(self, producto_id: int, formula_id: int, cantidad_programada: float, fecha_programada: str, lote_planeado: str, centro_trabajo: str = "", responsable: str = "", prioridad: str = "NORMAL", observaciones: str = "", estado_inicial: str = "BORRADOR") -> int:
        ...
        with self.conexion() as con:
            ...
            try:
                ...
                self._generar_requerimientos_conexion(con, orden_id, reemplazar=False)
                self._registrar_historial(con, orden_id, "", estado, "CREAR ORDEN", f"Orden {numero} creada con requerimientos automáticos.")
                con.commit()
                return orden_id
            except Exception:
                con.rollback()
    ...
    def _componentes_formula_conexion(self, con: sqlite3.Connection, formula_id: int) -> list[dict[str, Any]]:
        ...
        cco = self._columna_alias(cols, "costo_unitario_estandar", "costo_unitario", "costo", "costo_estandar")
        ct = self._columna_alias(cols, "tipo_componente", "tipo", "categoria")
    ...
    def _inventario_disponible_conexion(
        self,
        con: sqlite3.Connection,
        componente: str,
        presentacion: str,
    ) -> float:
        """Retorna el inventario físico disponible del componente."""
        if presentacion:
            fila = con.execute(
                """
    ...
    def _generar_requerimientos_conexion(
        self,
        con: sqlite3.Connection,
        orden_id: int,
        reemplazar: bool = False,
    ) -> list[dict[str, Any]]:
        """Calcula y persiste los requerimientos dentro de la transacción activa."""
        orden = con.execute(
            "SELECT * FROM ordenes_produccion_v2 WHERE id=?",
            (orden_id,),
        ).fetchone()
        if not orden:
            raise ErrorValidacion(f"No existe la orden {orden_id}.")

        existentes = int(con.execute(
            "SELECT COUNT(*) FROM ordenes_requerimientos_v2 WHERE orden_id=?",
            (orden_id,),
        ).fetchone()[0])

        consumido = float(con.execute(
            """
        ...
        if consumido > 0.000001:
            raise ErrorEstado(
                "No se pueden regenerar requerimientos de una orden con consumos registrados."
            )
        if existentes and not reemplazar:
            return [dict(f) for f in con.execute(
                "SELECT * FROM ordenes_requerimientos_v2 WHERE orden_id=? ORDER BY id",
                (orden_id,),
            ).fetchall()]

        formula = self._formula(con, int(orden["formula_id"]))
        base = float(formula["cantidad_base"] or 0)
        if base <= 0:
            raise ErrorValidacion(
                "La cantidad base de la fórmula debe ser mayor que cero."
            )
        factor = float(orden["cantidad_programada"] or 0) / base
        componentes = self._componentes_formula_conexion(
            con, int(orden["formula_id"])
        )
        if not componentes:
            raise ErrorValidacion(
                "La fórmula no tiene componentes; no se pueden generar requerimientos."
            )

        if existentes:
            con.execute(
                "DELETE FROM ordenes_requerimientos_v2 WHERE orden_id=?",
                (orden_id,),
            )

        resultado: list[dict[str, Any]] = []
        for componente in componentes:
            cantidad_neta = float(componente["cantidad"] or 0) * factor
            cantidad_teorica = cantidad_neta * (
                1 + float(componente["merma_pct"] or 0) / 100
            )
            cantidad_teorica = round(cantidad_teorica, 6)
            disponible = round(self._inventario_disponible_conexion(
                con,
                componente["componente"],
                componente["presentacion"],
            ), 6)
            costo_unitario = float(componente["costo_unitario"] or 0)
            costo_total = round(cantidad_teorica * costo_unitario, 2)
            estado = "DISPONIBLE" if disponible + 0.000001 >= cantidad_teorica else "FALTANTE"

            cur = con.execute(
                """
                INSERT INTO ordenes_requerimientos_v2(
                    orden_id, componente, presentacion, tipo_componente,
                    cantidad_teorica, cantidad_reservada, cantidad_consumida,
                    unidad, merma_pct, costo_unitario_estandar,
                    costo_total_estandar, disponible_creacion,
                ...
                (
                    ...
                    componente["componente"],
                    componente["presentacion"],
                    componente["tipo"] or "MATERIA PRIMA",
                    cantidad_teorica,
                    componente["unidad"] or "UND",
                    float(componente["merma_pct"] or 0),
                    costo_unitario,
                    costo_total,
                    disponible,
                    estado,
            ...
            resultado.append({
                ...
                "presentacion": componente["presentacion"],
                "tipo_componente": componente["tipo"] or "MATERIA PRIMA",
                "cantidad_teorica": cantidad_teorica,
                "cantidad_reservada": 0.0,
                "cantidad_consumida": 0.0,
                "unidad": componente["unidad"] or "UND",
                "merma_pct": float(componente["merma_pct"] or 0),
                "costo_unitario_estandar": costo_unitario,
                "costo_total_estandar": costo_total,
                "disponible_creacion": disponible,
    ...
    def generar_requerimientos(
        ...
        """Genera o regenera los requerimientos y registra la operación."""
        with self.conexion() as con:
            con.execute("BEGIN IMMEDIATE")
            try:
                orden = con.execute(
                    "SELECT numero, estado FROM ordenes_produccion_v2 WHERE id=?",
                    (orden_id,),
                ).fetchone()
                if not orden:
                    raise ErrorValidacion(f"No existe la orden {orden_id}.")
                estado = self._normalizar_estado(orden["estado"])
                if reemplazar and estado not in {
                    "BORRADOR", "PLANEADA", "APROBADA", "LIBERADA"
                }:
                    raise ErrorEstado(
                        f"No se pueden regenerar requerimientos de una orden {estado}."
                    )
                filas = self._generar_requerimientos_conexion(
                    con, orden_id, reemplazar=reemplazar
                )
                self._registrar_historial(
                    ...
                    estado,
                    estado,
                    "REGENERAR REQUERIMIENTOS" if reemplazar else "GENERAR REQUERIMIENTOS",
                    f"Se almacenaron {len(filas)} requerimientos para {orden['numero']}.",
                )
                con.commit()
                return filas
            except Exception:
