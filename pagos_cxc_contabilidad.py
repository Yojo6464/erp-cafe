"""
BME-ERP - Recaudos de Cuentas por Cobrar con Contabilidad
Archivo: pagos_cxc_contabilidad.py

Funciones:
- Consulta cuentas por cobrar pendientes.
- Registra abonos parciales o pagos totales.
- Actualiza saldo y estado de la cuenta.
- Aumenta el saldo del banco.
...
def preparar_estructura():
    ...
    try:
        ...
        agregar_columna(
            cursor, "pagos_cxc", "banco_id", "INTEGER"
        )
        agregar_columna(
            cursor, "pagos_cxc", "movimiento_banco_id", "INTEGER"
        )
        agregar_columna(
            cursor, "pagos_cxc", "comprobante_id", "INTEGER"
        )
        agregar_columna(
            cursor, "pagos_cxc", "comprobante_numero", "TEXT DEFAULT ''"
        )
        agregar_columna(
            cursor, "pagos_cxc", "estado_contable", "TEXT DEFAULT 'PENDIENTE'"
        )
        agregar_columna(
            cursor, "pagos_cxc", "observaciones", "TEXT DEFAULT ''"
        )
        agregar_columna(
            cursor, "pagos_cxc", "usuario", "TEXT DEFAULT ''"
        )
...
def seleccionar_cuenta(evento=None):
    ...
    cliente = str(datos[2])
    factura = str(datos[3])
    concepto = str(datos[4])
    saldo_texto = str(datos[6]).replace("$", "").replace(",", "")
    documento = str(datos[9])

    saldo = float(saldo_texto)

    lbl_cuenta_id.config(text=str(cuenta_id))
    lbl_cliente.config(text=cliente)
    lbl_factura.config(text=factura or "—")
    lbl_concepto.config(text=concepto)
    lbl_saldo.config(text=moneda(saldo))
    lbl_documento.config(text=documento or "—")
...
def limpiar_seleccion():
    ...
    lbl_factura.config(text="—")
    lbl_concepto.config(text="—")
    lbl_saldo.config(text="$0")
    lbl_documento.config(text="—")
    entrada_pago.delete(0, tk.END)
    tabla_historial.delete(*tabla_historial.get_children())
...
def obtener_datos_recaudo():
    seleccion = tabla_cuentas.selection()

    if not seleccion:
        raise ValueError("Seleccione una cuenta por cobrar.")
...
def registrar_recaudo(datos):
    ...
    try:
        ...
        if not cuenta:
            raise ValueError("La cuenta por cobrar ya no existe.")

        saldo_anterior_cxc = float(cuenta["saldo"] or 0)

        if saldo_anterior_cxc <= 0:
            raise ValueError("La cuenta ya está pagada.")

        if datos["valor_pago"] > saldo_anterior_cxc:
            raise ValueError(
                "El recaudo supera el saldo pendiente."
            )
        ...
        nuevo_saldo_cxc = saldo_anterior_cxc - datos["valor_pago"]

        if nuevo_saldo_cxc <= 0.005:
            nuevo_saldo_cxc = 0
            nuevo_estado = "PAGADA"
        else:
        ...
        return {
            ...
            "documento_cliente": cuenta["documento_cliente"],
            "factura": cuenta["factura"],
            "concepto": cuenta["concepto"],
            "saldo_anterior_cxc": saldo_anterior_cxc,
            "nuevo_saldo_cxc": nuevo_saldo_cxc,
...
def revertir_recaudo(resultado):
    conexion = conectar()
    ...
    try:
        ...
        cursor.execute("""
            ...
            WHERE id=?
        """, (
            resultado["saldo_anterior_cxc"],
            resultado["estado_anterior"],
            resultado["cuenta_id"]
        ))

        cursor.execute("""
            DELETE FROM pagos_cxc
            WHERE id=?
        """, (resultado["pago_id"],))
...
def registrar_pago():
    ...
    try:
        ...
        # BME-PROTECCION-PERIODOS - RECAUDOS-CXC
        validar_periodo_abierto(
            datetime.now().strftime("%Y-%m-%d")
        )
        resultado = registrar_recaudo(datos)

        numero_documento = (
            resultado["documento_cliente"].strip()
            or f"CLIENTE-{resultado['cuenta_id']}"
        )
        ...
        comprobante = contabilizar_evento(
            ...
            concepto=(
                f"Recaudo de cartera CxC #{resultado['cuenta_id']} - "
                f"{resultado['cliente']}"
            ),
            modulo_origen="CUENTAS_POR_COBRAR",
            tabla_origen="pagos_cxc",
            registro_origen_id=resultado["pago_id"],
            tercero={
                "tipo_documento": "NIT",
                "numero_documento": numero_documento,
                "nombre_razon_social": resultado["cliente"],
                "tipo_tercero": "CLIENTE",
                "origen_modulo": "CUENTAS_POR_COBRAR"
            },
            centro_costo=None,
            fecha=resultado["fecha"],
            documento_referencia=(
                resultado["factura"]
                or f"CXC-{resultado['cuenta_id']}"
            ),
            usuario=USUARIO
        )
        ...
        messagebox.showinfo(
            ...
            (
                ...
                f"Valor recibido: {moneda(resultado['valor_pago'])}\n\n"
                f"Saldo anterior CxC: "
                f"{moneda(resultado['saldo_anterior_cxc'])}\n"
                f"Nuevo saldo CxC: "
                f"{moneda(resultado['nuevo_saldo_cxc'])}\n"
        ...
        if resultado:
            try:
                revertir_recaudo(resultado)
                mensaje = (
                    "\n\nEl pago, la cartera y el movimiento "
                    "bancario fueron revertidos."
                )
...
ventana = tk.Tk()
ventana.title(
    "BME-ERP - Recaudos de Cuentas por Cobrar"
)
...
tk.Label(
    cabecera,
    text="RECAUDOS DE CUENTAS POR COBRAR",
...
tk.Label(
    cabecera,
    text="Cartera, bancos y contabilidad automática",
    font=("Segoe UI", 9),
    bg=COLOR_AZUL,
    fg="white"
...
columnas = (
    "ID", "Fecha", "Cliente", "Factura", "Concepto",
    "Valor", "Saldo", "Vencimiento", "Estado", "Documento"
)
...
lbl_cliente = dato("CLIENTE", 0, 1)
lbl_factura = dato("FACTURA", 0, 2)
lbl_saldo = dato("SALDO", 0, 3)
lbl_documento = dato("DOCUMENTO", 1, 0)
lbl_concepto = dato("CONCEPTO", 1, 1)
...
boton_registrar = tk.Button(
    panel_pago,
    text="Registrar y contabilizar recaudo",
...
tk.Label(
    ...
    text=(
        ...
        "Archivo de prueba: pagos_cxc_contabilidad.py"
    ),
    bg=COLOR_FONDO,
    fg=COLOR_SUAVE,
    font=("Segoe UI", 8)
