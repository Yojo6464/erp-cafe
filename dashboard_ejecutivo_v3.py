...
def actualizar():
    ...
    cuentas_cobrar = valor(
        cursor,
        "SELECT IFNULL(SUM(saldo),0) FROM cuentas_cobrar"
    )
...
tk.Label(frame1, text="Bancos").grid(row=0,column=0,padx=20)
tk.Label(frame1, text="Inventario").grid(row=0,column=1,padx=20)
tk.Label(frame1, text="Patrimonio").grid(row=0,column=2,padx=20)
...
tk.Label(frame4,text="Cartera Pendiente").grid(
    row=0,
    column=0,
    padx=20
)
